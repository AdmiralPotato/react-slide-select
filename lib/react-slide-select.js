...
/***/ function(module, exports, __webpack_require__) {
	...
	var SlideSelect = React.createClass({
		...
		determineIfSwipingByPoint: function determineIfSwipingByPoint(point) {
		...
		dragEnd: function dragEnd(point, event) {
			...
			slider.setState(newState, callback);
		},
		dragEndWithoutSnap: function dragEndWithoutSnap(point, event) {
			var slider = this;
			var newState = {
				startDragId: null,
				isSwiping: null
			...
			if (slider.state.isSwiping) {
				newState.x = slider.boundValue(0, slider.state.maxScrollPosition, slider.getAbsoluteXFromRelativeX(point.x));
			}
			slider.setState(newState);
		},
		...
		handleDrag: function handleDrag(eventName) {
			var slider = this;
			return function (syntheticDragEvent) {
